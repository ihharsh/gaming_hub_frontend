...
function ChatArea() {
  ...
  const sendMessage = () => {
    ...
    axios
      .post(
        "https://gaming-hub-project.onrender.com/message/",
        {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://gaming-hub-project.onrender.com/message/" + chat_id, config)
      .then(({ data }) => {
        setAllMessages(data);
        setloaded(true);
  ...
  const deleteChatsWithUser = async () => {
    try {
      const config = {
        headers: {
          Authorization: `Bearer ${userData.data.token}`,
      ...
      await axios.delete(
        `https://gaming-hub-project.onrender.com/chat/deleteChats/${userId}`,
        config
      );
